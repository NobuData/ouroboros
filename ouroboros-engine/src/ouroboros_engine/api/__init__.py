...
Seven routers today: :mod:`ouroboros_engine.api.health` (``/healthz``, the one path served
without the internal key), :mod:`ouroboros_engine.api.root` (``GET /``),
:mod:`ouroboros_engine.api.status` (``/v0/status``),
:mod:`ouroboros_engine.api.tasks` (``POST /v0/tasks/echo``),
:mod:`ouroboros_engine.api.estimate` (``POST /v0/estimate``, the first operation that does
work rather than demonstrating the shape of it),
:mod:`ouroboros_engine.api.workflows` (``POST /v0/workflows/validate`` and ``/dry-run``) and
:mod:`ouroboros_engine.api.plan` (``POST /v0/plan``, which drafts a batch of tickets). The
prefix the last five share and the compatibility rule that governs it are
:mod:`ouroboros_engine.api.v0`, which is where a router under ``/v0`` reads them from rather
than restating them.
