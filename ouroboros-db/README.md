...
> `V047` ([#300](https://github.com/NobuData/ouroboros/issues/300)) is mockup 10's right-hand
> column, and it is two thirds table and one third restraint. The **Changes so far** card gets
> `run_files` and `run_commits`; the **Resources** card gets one foreign key and nothing else.
> `run_files` is the interesting one: an executor reports its change-set *repeatedly* as it
> works, and each report states **where a file stands** rather than what has changed since it
> last spoke — so a write is an upsert on `(run_id, path)` that **replaces** the counts, and an
> ingestion path that added instead would produce a file reading `+228 -72` after six honest
> reports of `+38 -12`. `run_commits` is the same shape with the opposite ending, because a
> commit is immutable: it carries two unique keys — `(run_id, sha)`, the commit's identity, and
> `(run_id, seq)`, its place in the list — and a redelivered report is written as an *untargeted*
> `on conflict do nothing`, which covers both where a target would name one and let the other
> raise. `runs` gains `merge_strategy`, the snapshot of the pinned workflow's
> `open_pr_automerge` `merge_method` that the *will squash on merge* tag is, and
> `reserved_build_job_id`, the link the *forge-02 reserved* row reads through to the farm's own
> rows. **The restraint is the point of the rest.** Roadmap decision **R8** says the console
> *reads* the systems that already own its resource numbers — `token_usage` for tokens and cost,
> the route for the cap, `run_stages.token_budget` for the budget, `build_jobs` for the
> reservation, the stage clock for wall time — so no `total_additions`, no `file_count`, no
> token or cost column went onto `runs`, and `tests/constraints.sql` asks the catalogue by name
> that none of them ever does. A second counter is a counter that will one day disagree with the
> first one, and then somebody has to work out which is lying.

> `V048` ([#301](https://github.com/NobuData/ouroboros/issues/301)) is the bottom of that
> column and the buttons at the top of the page, and both tables are shaped by a failure mode
> rather than by a card. `guardrail_evaluations` holds the **Guardrails** card's verdicts, and
> the whole of its design is decision **R5**: *"secrets scan: fail"* is useless and
> *"fail — `drivers/can/telemetry_buf.c:214`, rule `aws-access-key-id`"* is actionable, but the
> obvious way to make evidence actionable is to include the matched text, which would write a
> live credential into this database and every backup of it. So `evidence` is closed by CHECK
> to `{path, line, rule_id, glob, detail}`, each key is typed and shaped, and **no string value
> may contain an unbroken twenty-character run of letters and digits** — which is not a
> catalogue of credential formats (V022 explains why that would be worthless) but a statement
> about what evidence *is*: a place and a rule, broken up by slashes, dots, hyphens and spaces.
> Every credential is such a run, because one that was not would be guessable. Re-evaluation
> appends rather than replaces, so `v_run_guardrails_latest` is the card's four rows and the
> table is the history behind them. `run_controls` is decision **R6**: *Abort run* is needed
> exactly when the executor is wedged, which is when a fire-and-forget RPC fails — so a control
> is a durable row with acknowledgments, its state machine is enforced by
> `run_controls_transition()` rather than by whichever service happens to be writing, terminal
> states accept no update at all, and **everything describing what was asked is fixed at
> insert**, because a queue whose entries can be edited in flight is a queue where *abort* was
> once *pause* and nobody can tell. Every control write lands in `audit_events` by trigger —
> which knowingly amends V022's *"nothing writes this table from SQL"*, because the criterion is
> that a row exists for **every** control and a rule the schema keeps cannot be forgotten by the
> next writer — with a body of four fields, three closed vocabularies and a boolean, so the
> steering text has nowhere to go rather than being redacted on the way past.

> `V049` ([#303](https://github.com/NobuData/ouroboros/issues/303)) is the first of the run
> console's migrations that arrives *with* its reader, because the ingestion contract and the
> schema it needs are one ticket. Everything an executor reports already had a table; what none
> of them had was a memory of the **request** that wrote them, and three of AP.1's acceptance
> criteria are about requests rather than about rows. `run_ingest_receipts` is that memory: the
> operation, the caller's idempotency key, a digest of what they sent and the answer they were
> given, unique per workspace. A receipt rather than a natural key, because four of the six
> operations have none a caller could supply — opening a run has no natural identity, a stage
> transition *moves* a row that already exists, an event batch is numbered by the server, and a
> change-set report is idempotent in its rows and emphatically not in the evaluation it
> triggers. The digest is what makes storing a response safe: a key reused with a *different*
> body is refused rather than answered with the first body's result, which would tell a caller
> its report landed when it had not. Two counters go on `runs` beside V046's: `event_hint`, the
> highest executor-side ordering number accepted — `run_events.seq` is assigned on arrival and
> therefore cannot notice two batches that overtook one another, where the executor's own
> counter can — and `change_set_seq`, the number `guardrail_evaluations.change_set_seq` is
> allocated from, which V048 stores and deliberately does not assign. A run standing at `0` on
> the second has reported nothing, which is the schema half of *"a run with no file changes
> triggers no evaluation"*.
>
> `V050` ([#306](https://github.com/NobuData/ouroboros/issues/306)) is AP.4's two amendments
> to that queue. `runs.status` gains **`canceled`**, a fourth terminal status, because an
> acknowledged abort must leave the run terminal and none of V008's three is true of it:
> `failed` would draw a person's decision as the agent's failure, and `needs_human` would say
> the run is waiting for somebody. `runs_status` and `runs_terminal_finished_at` are widened
> under their V008 names. `run_controls` gains **`remember`**, the steer's explicit *remember
...
> `truncation_note`.
>
> `V056` ([#353](https://github.com/NobuData/ouroboros/issues/353)) adds mockup 12's
> **Verification gates** card as rows (decision **V2**). `pr_gate_definitions` is the gate set a
> PR is held to, materialized from the pinned policy and org config. `gate_key` is one of the
> seven built-in keys or `custom:<name>`, so a new gate kind needs no migration, and `source`
> records which policy or config produced each gate. `pr_gate_results` holds one verdict per
> gate per revision, and a re-evaluation appends a row rather than overwriting one, so Revision
> 1's two red gates stay inspectable after Revision 2 turns them green.
> `pr_gate_results_latest` is the card's read: one row per gate per revision. The verdict
> vocabulary has six values. `waived` and `not_required` are policy, and **`unavailable`** (no
> provider yet) is kept distinct from `pending` (a provider evaluating).
> `pr_gate_aggregate(revision_id)` computes `x of y green` over the *required* definitions and
> the merge precondition: `waived` and `not_required` satisfy it, `pending` and `unavailable` do
> not. `evidence_ref` is a typed `{kind, id}` link resolved in the PR's workspace at write. The
> kinds `vote` and `approval` are reserved and refused until AZ.1 and AX.5 add their tables.
>
> `V057` ([#354](https://github.com/NobuData/ouroboros/issues/354)) adds mockup 12's
> **acceptance-criteria matrix** and **review thread** as rows (decision **V6**). `pr_criteria`
> holds one quoted claim per row, with a `source` (`plan`, `manual`, or `extracted`, which is
> reserved and refused until AZ.2) and a `status`. A claim can only become `verified` when it has
> at least one evidence row, and it is `waived` exactly when `waiver_ref` names an AS.4 waiver of
> the PR's own run. `pr_criteria_evidence` stores **typed references**, never free text:
> `test_case`, `hil_measurement`, `build_artifact` (an uploaded `test_artifacts` row), `hunk` (a
> path in the revision's files snapshot, with a `1 ≤ start ≤ end` line range) and `analysis_note`
> (tied to a revision). A dangling reference, or one from another workspace or PR, is refused at
> write. References cascade, so evidence leaves with the row it cited. When a verified claim loses
> its last evidence row, it drops back to `unverified`. `pr_thread_entries` records who wrote each
> entry in `author_kind` (`model`, `policy_bot` or `human`). Until AZ.1, a `model` entry must carry
...
> the trail.
>
> `V059` ([#328](https://github.com/NobuData/ouroboros/issues/328)) gives mockup 11's
> `coverage 87.4% (+0.6%)` something to compute from (decision **T9**). A `coverage` artifact
> carries the two counts its report parses to — `lines_covered` and `lines_total` — and never a
> percentage. `test_run_coverage` divides them per attempt, summing an attempt's reports, and
> takes the delta against the previous attempt of the same run that has coverage.
>
> `V060` ([#330](https://github.com/NobuData/ouroboros/issues/330)) is how those artifacts get
> there: the build farm's **job-scoped upload** (decision **T4**). `build_job_artifact_uploads` is
> one row per build job — while open, the SHA-256 of the single-use token its offer carried (never
> the token); once closed, the upload's receipt: the attempt it filled, the **manifest** of every
> collected file (`stored`, `truncated` or `skipped`, with the reason) and the job **warnings** the
> page renders, a quota breach among them. It is `build_jobs`' result linkage. A re-offer re-mints
> an open ledger; a closed one is final (`build_job_artifact_uploads_lifecycle`), except the
> attempt reference its deletion clears. `runner_pools.artifact_globs` and
> `build_jobs.artifact_globs` (the pool's plus the submission's, snapshotted at submit) name what a
> build collects beyond the built-in result set, held by `artifact_globs_valid()` to paths inside
> the workspace.
...
>
> **One more ticket, and it is not mockup 09's: `#482`** (`5eed0030…`,
> [#307](https://github.com/NobuData/ouroboros/issues/307)). It is mockup 10's ticket, the one
> the dashboard seed's live run is for. It is mirrored so the simulated-run driver can open runs
> against it, because `POST /internal/runs` opens a run for a **ticket** and the console's run
> was seeded straight into `runs`. It shares its number with that run on purpose. It is
> **closed**, so none of the open-ticket figures above moves (opening a run does not read a
> ticket's state), and it sits on no lane and in no dependency. The planning seed's own counts
> select the `5eed001d…` prefix, so they still read 52.
...
│   ├── V047__run_changes_and_links.sql     # run_files (cumulative, upserted), run_commits, runs.merge_strategy and the farm reservation — #300
...
│   ├── V059__test_coverage_summary.sql # test_artifacts.lines_covered/lines_total on coverage reports, and test_run_coverage (percent + delta vs the previous attempt) — #328
│   ├── V060__job_artifact_uploads.sql # build_job_artifact_uploads (the single-use upload token's ledger, then the receipt: attempt, manifest, warnings) and the artifact globs on runner_pools and build_jobs — #330
│   ├── V061__classification_routing.sql # the Mark & Route actions' columns: failure_classifications.subtype (unclear_requirements), run_controls.retry_stage (the correction round), the runner health note and build_jobs.test_selection — #332
...
| `runs` | `V008`, `canceled` added `V050` | One run of the loop against one issue — the dashboard read-model | `status` is one of `coding\|building\|review\|merged\|needs_human\|failed\|canceled` (`canceled` is an aborted run — [#306](https://github.com/NobuData/ouroboros/issues/306)), and a terminal status carries `finished_at` exactly when it is terminal; the run's repository must belong to the run's organization. `loop_seq` (`V045`, [#298](https://github.com/NobuData/ouroboros/issues/298)) is the `Loop #1847` counter — unique per workspace and allocated by `runs_allocate_loop_seq()` under an advisory lock when an insert supplies none — beside `branch_name` and `workflow_version_pin`, the version half of the pin whose slug half is `workflow_tag`. `V046` ([#299](https://github.com/NobuData/ouroboros/issues/299)) adds the transcript's accounting — `simulated`, decision **R4**'s watermark that every `run_events` row inherits and that is fixed once the run has written one; `event_cap` and `event_byte_cap`, the two caps bounding it; and `event_seq`, `event_bytes` and `events_elided_at`, which are `run_events_append()`'s alone, because a writer that could set them would be telling the store how much of itself it had used. `V047` ([#300](https://github.com/NobuData/ouroboros/issues/300)) adds the last two the console reads: `merge_strategy`, the snapshot of the pinned workflow's `open_pr_automerge` `merge_method` behind the *will squash on merge* tag — null when the pinned terminal opens no pull request, and the card then draws no tag — and `reserved_build_job_id`, a **composite** reference onto `build_jobs (id, organization_id)` so a run can only reserve a build of its own workspace, released with `on delete set null (reserved_build_job_id)` because an unqualified `set null` on a composite reference would try to null `organization_id` too and fail the delete instead |
...
| `run_files` | `V047` | Where each file of a run's change-set stands, **now** ([#300](https://github.com/NobuData/ouroboros/issues/300), AO.3) — the file rows of mockup 10's *Changes so far* card | `(run_id, path)` unique, which is the key a report **upserts** on: the counts are cumulative against the run's base and a report *replaces* them, because a delta added to a cumulative row is a count that climbs forever; `path` is repository-relative and segment-clean (no leading `/`, no `..`), because AO.4's allowed-path rules are matched against this column; `status` is one of `added\|modified\|deleted\|renamed` and is held to the counts beside it — an `added` file removes nothing and a `deleted` one adds nothing; counts cannot go negative; and there is **no `updated_at`**, because a report is the only thing that writes this table and `last_reported_at` is when it happened |
| `run_commits` | `V047` | The commits a run has made, in the order its card draws them ([#300](https://github.com/NobuData/ouroboros/issues/300), AO.3) | Two unique keys — `(run_id, sha)`, the commit's identity, and `(run_id, seq)`, its place in the list — so a **redelivered report is a no-op** and the order is a property of the rows; the write is an *untargeted* `on conflict do nothing`, since a target names one key and lets the other raise; `sha` is lower-case hex of seven to forty characters, so one commit cannot become two rows by case or by abbreviation width; `seq` is the writer's order rather than the clock's, because a rebase rewrites `committed_at` and a list that reordered itself when the loop rebased would disagree with the branch it describes |
...
| `test_run_coverage` | `V059` | One row per attempt with coverage: lines covered and instrumented, `percent` to one decimal, and `delta` against the latest earlier attempt of the run with coverage | A view — mockup 11's `coverage 87.4% (+0.6%)` computed from counts; the delta is taken from the unrounded ratios and rounded once; a tombstoned report still counts |
...
| `test_artifacts` | `V055` | One file an attempt uploaded (option **3-A**): `name`, `kind`, `size_bytes`, `storage_ref`, `checksum`, `retained_until`, `expired_at`, `truncated` and `truncation_note` | Composite reference onto `test_runs` with cascade; `(test_run_id, name)` unique; `kind` is `junit\|hil\|coverage\|log\|capture\|other`; `storage_ref` is `{"driver", "key"}`, with the driver as data (`local`, `s3`, …); `checksum` is `<algorithm>:<hex>`; `truncation_note` is present exactly when `truncated`; `test_artifacts_retention_idx` serves the sweep (`expired_at is null`, by `retained_until`); only `storage_ref`, `retained_until` and `expired_at` change, and nothing changes after expiry (`test_artifacts_lifecycle`); `ouroboros_app` cannot delete, so expiry is a tombstone. `V059` adds `lines_covered` and `lines_total`, required exactly on a `coverage` artifact (covered ≤ total, total > 0) and frozen with the file |
| `build_job_artifact_uploads` | `V060` | A build job's artifact upload ([#330](https://github.com/NobuData/ouroboros/issues/330), AT.2, decision **T4**): while open, `token_hash` (SHA-256 of the single-use token), `minted_at` and `expires_at`; once closed, `closed_at`, `test_run_id`, the `manifest` of every collected file, the job `warnings` and `stored_bytes` | Keyed by `build_job_id`, a composite reference onto `build_jobs` with cascade — `build_jobs`' result linkage. `token_hash` is 64 lowercase hex, so a token can never be stored as itself. `build_job_artifact_uploads_receipt` makes the receipt exist exactly when the upload closed. An open ledger is re-minted in place by a re-offer; once closed nothing changes but the attempt reference, which `on delete set null (test_run_id)` clears (`build_job_artifact_uploads_lifecycle`). `ouroboros_app` mints and closes but cannot delete |
...
| `pr_gate_definitions` | `V056` | One gate a PR is held to ([#353](https://github.com/NobuData/ouroboros/issues/353), AW.2, decision **V2**) — a row of mockup 12's *Verification gates* card: `gate_key`, provenance `source`, `required`, `sort_order`, `label` | `(pr_id, gate_key)` unique; `gate_key` is `build\|test_suite\|physical_hil\|diff_vs_plan\|secrets_license\|model_review\|human_approval` or `custom:<name>` (lowercase); `source` and `label` are not blank; `pr_id` and `gate_key` are frozen once written; cascades from the PR; `ouroboros_app` may not delete |
| `pr_gate_results` | `V056` | One evaluation of one gate on one revision: `verdict`, the `evidence` mono line, the typed `evidence_ref`, `evaluated_at`, `provider_version` | `verdict` is `green\|red\|pending\|waived\|not_required\|unavailable`, and `unavailable` is never `pending`; `(definition_id, revision_id, evaluated_at)` unique, so re-evaluation appends; the revision must be the definition's PR's; `evidence` is at most 512 characters; `evidence_ref` is exactly `{kind, id}` and resolves, via the `security definer` `pr_gate_evidence_ref_resolves()`, to a `build_jobs`, `test_runs`, `hil_measurements` or `guardrail_evaluations` row of the PR's workspace, while `vote` and `approval` are reserved and refused; append-only to `ouroboros_app` |
| `pr_gate_results_latest` | `V056` | The newest result per gate per revision — the card's read | A `DISTINCT ON` view; `ouroboros.pr_gate_aggregate(revision_id)` computes `required_count` (y), `green_count` (x), `red_count`, `satisfied_count` (green, waived, not_required) and `merge_ready` from it |
...
**`v_run_guardrails_latest`** (`V048`) is the Guardrails card's four rows: `distinct on
(run_id, check)` over `guardrail_evaluations`, ordered by `evaluated_at desc` with the
change-set number and the row id behind it so *latest* is deterministic rather than merely
usually stable. A run reports its change-set many times and each report re-runs the four
checks, so the table grows and this stays four rows — and the rows it passes over are still
there, which is what makes *fail → fixed → pass* a sequence somebody can recover rather than a
verdict that was overwritten. It is read-only for a reason worth stating: a write through it
would be an attempt to *replace* a verdict, which is the one thing that table is shaped to
prevent.
