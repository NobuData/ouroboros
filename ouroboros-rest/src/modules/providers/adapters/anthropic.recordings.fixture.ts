/**
 * The Anthropic adapter's **recorded fixtures** — the captured responses themselves.
 *
 ...
 * `fetch` over a captured response and calls the adapter"*. The captures are here; the
 * stand-in `fetch` is `http.recordings.fixture.ts`, shared with every other HTTP adapter since
 * AC.3 ([#218](https://github.com/NobuData/ouroboros/issues/218)). Neither opens a socket, so
 * `anthropic.conformance.spec.ts` and `anthropic.adapter.spec.ts` run in `yarn test` — the
 * suite that will actually notice them — rather than in an integration suite gated on somebody
 * having a key.
 *
 * ```
