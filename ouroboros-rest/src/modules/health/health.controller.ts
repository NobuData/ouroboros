 ...
 *
 * Both are `@AllowAnonymous()` too, and for the third time the same reason: their reader is
 * a container platform, which holds no session and could not be given one
 ...
 *
 * The decorator is on the *class*, so it covers both routes and any probe added beside
 * them. It is the library's, and it carries exactly the exemption `@Public()` carried
 * before [#703](https://github.com/NobuData/ouroboros/issues/703) swapped the guard that
 * reads it.
 */
@AllowAnonymous()
