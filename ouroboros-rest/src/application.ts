...
import { INSTALLER_PATHS } from "./modules/farm/installer/installer.paths";
 ...
 *     `src/modules/internal/internal.paths.ts`. The runner installer
 *     ([#248](https://github.com/NobuData/ouroboros/issues/248)) is the fourth: its reader is
 *     `curl` piping `/install.sh` into a shell on a build machine, in a command copied once and
 *     kept for years, so it answers where every such installer answers — see
 *     `src/modules/farm/installer/installer.paths.ts`.
...
export function configureApplication(app: INestApplication): void {
  ...
  // one; `PROBE_PATHS`, `INTERNAL_PATHS`, `INSTALLER_PATHS` and `AUTH_PREFIX_EXCLUSIONS`
  // stay readonly, so nothing can add a route to the set that escapes the prefix by
  // mutating it from somewhere else.
  //
  ...
  app.setGlobalPrefix(API_PREFIX, {
    exclude: [...PROBE_PATHS, ...INTERNAL_PATHS, ...INSTALLER_PATHS, ...AUTH_PREFIX_EXCLUSIONS],
  });
