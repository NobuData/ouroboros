...
| #23 `ouroboros-db: [3.5] Dev seed data` | ~~Amended by B.4~~ — **shipped as originally specified** (`R__dev_seed.sql`), for the same reason #21 did: it seeds the tables #20–#22 actually built. B.4 becomes a rewrite of that one file if BetterAuth is confirmed; its production guard (`${ouro_dev_seed}`, false everywhere but the dev stack), its `5eed…` id convention and its two test files carry over unchanged. |
