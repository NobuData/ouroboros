...
**258 of 454 ordered issues are closed** — P0 through P9 are complete, and P10 is under way.
...
| **P10** | Run console & ingestion contract | 🟡 13/17 | 17 | 55 | Mockup 10 |
...
> **Status:** ✅ **Complete** — all 50 issues closed
...
> **`N.2` · [`#116`](https://github.com/NobuData/ouroboros/issues/116) has shipped, and row `160`
> leaves the table.** Mockup 03's filter bar is a query-string editor in `ouroboros-ui` 0.53.0:
> every control in `?repo=&labels=&state=&sort=&q=` (decision K8), read on the server so the first
> paint is the filtered view and written with `router.replace` so **Back** is the page before. The
> chips are M.1's facets on `<button aria-pressed>` with the mockup's `chip-on` treatment keyed to
> the attribute; the repository select is H.1's focus repository seen from the page, kept in step
> both ways; the search waits 300 ms and every other control flushes it; **Clear all** goes to
> `/issues`, the default view's one address.
>
> **The head reads twice now.** M.1 scopes its `meta` by `repo`, so the head's counts follow the
> select through a *view* read asked with the bar's query; L.4's claim carries no `repo`, so the
> confirmation's mirrored count keeps its `state=all` *scope* read beside it. 75 new cases in two
> suites. P6 is 18 of 23, and the document stands at **160 of 454**.

> **`N.3` · [`#117`](https://github.com/NobuData/ouroboros/issues/117) has shipped, and row `161`
> leaves the table.** Mockup 03's backlog table is the page's core in `ouroboros-ui` 0.54.0: the
> seeded nine drawn row for row under `sort=effort`, a checkbox column writing the selection store
> the head's **Queue N selected ⟳** reads (select all, none and one, surviving filter and page
> changes), the four status pills in the ticket's hues with M.1's fifth state `unsized` drawn under
> its own word, the freshness tag pressing M.4's sync, and a pagination footer riding `&page=`
> beside K8's five controls.
>
> **The statuses are live because #87's loop became generic.** `app/poll.ts` is the dashboard's
> polling loop over any payload, `app/api/poll-response.ts` its answer-to-HTTP translation, and
> `GET /api/backlog` a second route handler on this origin — so an `estimating…` row becomes `sized`
> on the poll after the pipeline finishes, which was watched on the composed stack through that
> handler when L.4's sweep re-queued the seeded `#483`. The #46 Table grew a checkable shape whose keyboard keeps *move*,
> *check* and *activate* apart. Two of the mockup's cells were corrected rather than copied: an
...
> **`N.5` · [`#119`](https://github.com/NobuData/ouroboros/issues/119) has shipped, and row `163`
> leaves the table.** Mockup 03's `ISSUE DETAIL` panel in `ouroboros-ui` 0.56.0: beside the table
> in the mockup's grid, stacking under it at the mockup's break, and drawn from M.2's one-issue read
> through a poll of its own — `GET /api/backlog/{id}`, a third route handler on this origin, over
> the same loop the table keeps, now keyed on whatever it asks for. The head is the row's own facts
> the moment a row is opened; the body is the last answer about it, across all four sizing states:
> the excerpt, the *AI Work Breakdown* with the mockup's `~180k` and `12–18 min`, the risk meter in
> the level's hue and length, **Queue for loop** (M.3 with one id), **Re-estimate** (L.4's single,
> the fifth Server Action, inert while `estimating…`), **Open on GitHub ↗** in a new tab, and the
> collapsible trace built from the trace's own fields — *sized by heuristic-v0 · 2m ago* over
> *signals: none recorded by this estimator*, never the mockup's model name (decision **K10**).
>
> **A press asks every poll now.** Re-estimate and the single queue publish the same *ask again*
> signal the workspace switch does, so the panel draws the `estimating…` the service has already
> written at once, the table's pill follows, and the new version renders when the pipeline finishes
> — no reload. Two corrections are argued in the intake roadmap: the seeded `#485` is queued by the
> dashboard seed, so its pill reads `queued` and **Queue for loop** is inert with the reason where the
> mockup draws `sized`; and the seed writes the mockup's three files for `#485`, so the *file estimate
> arrives with the full estimator* sentence is what a live `heuristic-v0` estimate draws, not the
...
> **Status:** 🟡 **In progress** — 13 of 17 issues closed
