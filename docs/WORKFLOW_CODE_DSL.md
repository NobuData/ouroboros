...
| **#179**, the intelligence fixtures | The span map, the diagnostics stream, the Loop Checks rows and the symbol table's scopes, held for every seeded workflow to [`fixtures/code-intelligence/`](../schemas/workflow-dsl/fixtures/code-intelligence) (`code.intelligence.spec.ts`, and the served payloads in `code.intelligence.integration-spec.ts`). A print that moves a line fails there, naming the golden and `OURO_UPDATE_GOLDENS=1`, and the TypeScript compiler reads each stage call's lines independently of the printer's arithmetic. |
