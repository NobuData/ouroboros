...
**It installs from the deployment it will talk to**
([#248](https://github.com/NobuData/ouroboros/issues/248)). `ci/runner` publishes each version as
a GitHub release — three binaries, `install.sh`, and a `SHA256SUMS` over them — and an operator
copies a release into `OURO_FARM_RELEASES_DIR`, from which `ouroboros-rest` serves
`GET /install.sh` (its own address written into the script) and the release's files. So the build
farm's one-liner, `curl -fsSL 'https://<deployment>/install.sh?version=<v>' | sh -s -- …`, needs a
route to nothing but the control plane the agent is about to dial anyway, and trusts no public
host: mockup 08's `get.ouroboros.dev` is design shorthand. The script verifies the binary against
`SHA256SUMS` before running anything, enrols it, and leaves it under a systemd unit or a launchd
daemon that restarts it on failure and starts it at boot.

The other end of that connection is `ouroboros-rest`'s agent gateway
([#251](https://github.com/NobuData/ouroboros/issues/251), `src/modules/farm/gateway/`): it
authenticates the upgrade against the farm CA's revocation list before any socket exists,
records each runner's `hello` and heartbeats into the `runners` row the farm page reads, infers
`offline` from three missed heartbeats without ever moving `last_seen_at` off the last real one,
and applies each `job.finish` exactly once through a ledger keyed on its envelope id. Its
sessions live in the process that holds the socket; everything the farm page renders is in the
database.
...
| `OURO_RUN_SIMULATOR_SECRET` | `ouroboros-rest`, `ouroboros-engine` | The **second** value `X-Ouro-Internal-Key` may carry, and the one that marks the runs it opens as simulated (AP.1, [#303](https://github.com/NobuData/ouroboros/issues/303), decision **R4**). A run's `simulated` watermark follows the *principal*, and the only thing a caller proves on this channel is which secret it holds — so a second principal needs a second secret. Optional: unset is a deployment that runs no simulator, and a value equal to `OURO_ENGINE_SHARED_SECRET` is refused at boot, because two principals presenting one proof are one principal | `dev-run-simulator-secret-change-me` |
...
| `OURO_GITHUB_API_BASE_URL` | `ouroboros-rest` | Where GitHub's REST API is, when it is not `https://api.github.com` ([#288](https://github.com/NobuData/ouroboros/issues/288)). `github.octokit.ts` has carried the parameter for GitHub Enterprise Server since K.3; this is the setting that reaches it. It moves the address only — the workspace's token, the rate-limit rules and every failure classification are unchanged. The other thing that sets it is the e2e compose override, which points it at the suite's sandbox tracker | `https://api.github.com` |
...
| `OURO_FARM_MIN_AGENT_VERSION` | `ouroboros-rest` | The oldest `ouroboros-runner` build the farm's agent gateway accepts, as a semantic version ([#251](https://github.com/NobuData/ouroboros/issues/251)) — the GitHub-runner pattern. The agent runs on customer machines and cannot be force-upgraded, so the right to refuse an old one exists from the first release: a `hello` reporting a lower version is answered `refuse {version.below_minimum}` with a sentence naming the floor, and the agent exits rather than reconnecting into it. Compared by SemVer precedence, so `0.10.0` is newer than `0.9.0` and a pre-release precedes its release; validated as SemVer at boot. The protocol-*line* floor is separate and belongs to the build ([`RUNNER_PROTOCOL.md` § 3](RUNNER_PROTOCOL.md#3-versioning-and-the-version-floor)) | `0.0.0-0` — the lowest SemVer version, so no effective floor; unset means the same |
| `OURO_FARM_PUBLIC_URL` | `ouroboros-rest` | The https origin runner machines reach this deployment at ([#248](https://github.com/NobuData/ouroboros/issues/248)). `GET /install.sh` writes it into the installer as its `--server`, so the build farm's one-liner downloads the agent from this deployment and enrols it into this deployment — no public host between a build machine and the binary it runs. An origin, https only, because the agent speaks TLS and nothing else; validated at boot | `OURO_REST_URL` when that is https; otherwise the installer declines, naming this variable |
| `OURO_FARM_RELEASES_DIR` | `ouroboros-rest` | The directory `ouroboros-rest` serves `ouroboros-runner` releases from ([#248](https://github.com/NobuData/ouroboros/issues/248)) — one subdirectory per version, as `make release` writes `dist/<version>/` and the GitHub release `ouroboros-runner-v<version>` carries it. Read on every request, so a release copied in is served without a restart; a relative path is resolved against the service's working directory | unset: no installer is served. The template's `../ouroboros-runner/dist` is `make release`'s output for `yarn dev` |
| `OURO_FARM_LOG_BUDGET_BYTES` | `ouroboros-rest` | The bytes of finished builds' logs one workspace keeps ([#253](https://github.com/NobuData/ouroboros/issues/253), decision **B8**). Build logs live in PostgreSQL, so the retention sweep holds each workspace under this budget by removing the oldest finished jobs' logs — whole, never in part — beside the thirty-day age rule; a running build's log is never swept. A whole number of bytes, 1 MiB to 1 TiB | `2147483648` (2 GiB) |
| `OURO_ARTIFACT_STORE` | `ouroboros-rest` | Which driver keeps build jobs' uploaded artifacts ([#330](https://github.com/NobuData/ouroboros/issues/330), option **3-A**): `local`, a volume this service mounts, or `s3`, any S3-compatible store — S3 or MinIO. A local volume does not scale horizontally, so a deployment that runs replicas switches to `s3`, by configuration alone; each `test_artifacts` row records the driver it was written with | `local` |
| `OURO_ARTIFACT_DIR` | `ouroboros-rest` | The local store's directory ([#330](https://github.com/NobuData/ouroboros/issues/330)); relative paths resolve against the working directory. Read only with the `local` driver | `.artifacts` — gitignored in a checkout; `/app/.artifacts` in the image, owned by its service user |
| `OURO_ARTIFACT_S3_ENDPOINT` | `ouroboros-rest` | The S3-compatible endpoint's http(s) origin ([#330](https://github.com/NobuData/ouroboros/issues/330)), addressed path-style. Required with the `s3` driver, like the next three; boot fails naming whichever is missing | *(unset)* — the template writes the compose `minio` profile's |
| `OURO_ARTIFACT_S3_BUCKET` | `ouroboros-rest` | The bucket artifacts are written to ([#330](https://github.com/NobuData/ouroboros/issues/330)). Must already exist — the driver never creates one | *(unset)* |
| `OURO_ARTIFACT_S3_REGION` | `ouroboros-rest` | The region S3 requests are signed for (SigV4, [#330](https://github.com/NobuData/ouroboros/issues/330)) | `us-east-1` |
| `OURO_ARTIFACT_S3_ACCESS_KEY_ID` | `ouroboros-rest` | The S3 access key id ([#330](https://github.com/NobuData/ouroboros/issues/330)) | *(unset)* |
| `OURO_ARTIFACT_S3_SECRET_ACCESS_KEY` | `ouroboros-rest` | The S3 secret access key ([#330](https://github.com/NobuData/ouroboros/issues/330)) — **a secret**, redacted from the boot log | *(unset)* |
| `OURO_ARTIFACT_QUOTA_BYTES` | `ouroboros-rest` | The bytes of live artifacts one workspace keeps ([#330](https://github.com/NobuData/ouroboros/issues/330)). A file that would cross it is not stored, and the job carries an `artifact_quota_exceeded` **warning** — never a failed build. 1 KiB to 1 PiB | `10737418240` (10 GiB) |
| `OURO_ARTIFACT_MAX_FILE_BYTES` | `ouroboros-rest` | The per-file cap every job offer carries ([#330](https://github.com/NobuData/ouroboros/issues/330)); a larger file is uploaded cut to it and listed as truncated. 1 KiB to 64 GiB | `67108864` (64 MiB) |
| `OURO_ARTIFACT_MAX_JOB_BYTES` | `ouroboros-rest` | The per-job cap every job offer carries ([#330](https://github.com/NobuData/ouroboros/issues/330)); files past it are listed as skipped. At least the per-file cap | `268435456` (256 MiB) |
| `OURO_ARTIFACT_RETENTION_DAYS` | `ouroboros-rest` | How many days an uploaded artifact is kept — its `retained_until`, mockup 11's `retained 30d` ([#330](https://github.com/NobuData/ouroboros/issues/330)). 1 to 3650 | `30` |
| `OURO_RUNNER_SERVER` | `ouroboros-runner` | The control plane the agent enrols with and connects to, `https://` only ([#244](https://github.com/NobuData/ouroboros/issues/244)). A fallback for `--server`: **the agent reads no `.env` file** — it runs on a customer's machine, configured by its service unit — so its six variables are here because this registry is the complete list, not because a checkout's `.env` configures it | `https://localhost:8443` |
...
| `OURO_RUNNER_NO_SHELL` | `ouroboros-runner` | `true` runs no job directly on this machine (`--no-shell`, [#246](https://github.com/NobuData/ouroboros/issues/246)): the hello reports `shell: false` and shell offers are declined, while container jobs still run. Shell jobs run unsandboxed as the agent's user — the tenant's machine running the tenant's command — so a machine holding signing keys opts out here. Anything but a boolean stops the agent | `false` |
| `OURO_RUNNER_KEEP_WORKSPACE_ON_FAILURE` | `ouroboros-runner` | `true` leaves the workspace of a job that failed, timed out or errored under `<state-dir>/work/<job id>` for diagnosis (`--keep-workspace-on-failure`, [#246](https://github.com/NobuData/ouroboros/issues/246)); by default every workspace is removed when its job ends | `false` |
| `OURO_RUNNER_LOG_CAP_BYTES` | `ouroboros-runner` | The most of one job's output the agent ships (`--log-cap-bytes`, [#247](https://github.com/NobuData/ouroboros/issues/247)), 65536–268435456 — V040's own `build_jobs.log_cap_bytes` range. The head of the log is kept and the rest reported in `job.finish.log.dropped_bytes`, which the run console renders as one elision marker with the control plane's own cap. Outside the range, or not a number, stops the agent | `67108864` |
...
| `OURO_BACKLOG_SYNC_INTERVAL_SECONDS` | `ouroboros-rest` | Whole seconds between backlog sync cycles ([#102](https://github.com/NobuData/ouroboros/issues/102), decision **K2**) — what mockup 03's *"synced 40s ago"* tag counts from. An incremental cycle asks each enabled repository for the issues updated since its stored watermark, so it costs roughly one request per repository from one token's hourly GitHub budget. Jittered ±25% like the sweeps above, and with the same floor argument in the other direction: a minute rather than ten seconds, because this knocks on github.com rather than on the operator's own network. There is deliberately no value that turns polling off | `300` |
| `OURO_ESTIMATION_CONCURRENCY` | `ouroboros-rest` | How many issues the estimation pipeline sizes at once ([#107](https://github.com/NobuData/ouroboros/issues/107), decision **K7**) — the bound on outbound `POST /v0/estimate` calls, and on the pool connections their writes hold. It is what keeps a cold import of a large backlog from opening one engine connection per issue: the work queue admits this many at a time and the rest wait in memory. Raising it far past the database pool's ten connections buys nothing, because the extra work then queues on the pool instead | `4` |
| `OURO_ESTIMATION_CONFIDENCE_FLOOR` | `ouroboros-rest` | Below what confidence, as a percentage, an estimate sends its issue to `needs_human`. The transition is the control plane's policy rather than the estimator's: `ouroboros-engine` reports how sure it is and the L.1 contract has no `needs_human` field at all ([#105](https://github.com/NobuData/ouroboros/issues/105)). The default is the number the bundled heuristic's tables were calibrated against, restated here so an installation can disagree without a release — `0` accepts every estimate, `100` sends every issue to a person. An estimate below the floor is still stored in full, trace and all | `70` |
| `OURO_ESTIMATION_STALE_SECONDS` | `ouroboros-rest` | How long an issue may sit in `estimating` before the recovery sweep re-queues it. Not a timeout on an estimate — the engine gateway's own deadline is that — but how long a row may claim to be estimating with nothing estimating it, which is what a process killed mid-flight leaves behind. There is deliberately no value that turns the sweep off: a stranded row is invisible rather than wrong, and invisible is worse | `600` |
| `OURO_ESTIMATION_SWEEP_INTERVAL_SECONDS` | `ouroboros-rest` | Whole seconds between those recovery sweeps, jittered ±25% like every other loop in the service. Faster than the two above it because it knocks on a different door: one indexed query against this deployment's own database, which on a healthy service returns nothing. A row stranded by a restart comes back within this plus `OURO_ESTIMATION_STALE_SECONDS` | `120` |
| `OURO_RUN_CONTROL_TTL_SECONDS` | `ouroboros-rest` | Whole seconds a pause, resume or abort is worth delivering (AP.4, [#306](https://github.com/NobuData/ouroboros/issues/306), decision **R6**). A control the executor has not acknowledged by then is `expired`, which the console renders as *"no response — the run may be between stages"* rather than as success. 10–3600 | `120` |
| `OURO_RUN_STEER_TTL_SECONDS` | `ouroboros-rest` | The same for a steer, and longer, because a steer lands at the executor's next point of context injection and a nudge that arrives a little late is still the nudge. 10–3600 | `300` |
| `OURO_RUN_CONTROL_SWEEP_SECONDS` | `ouroboros-rest` | Whole seconds between control-expiry sweeps, jittered ±25%. The routes sweep their own run first, so this only decides how soon an expiry reaches the audit trail for a run nobody is watching. 5–3600 | `15` |
