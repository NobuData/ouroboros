...
import { issuesReadings } from "../helpers/issues";
import { membership, sessionUser } from "../helpers/login";

/**
 * The intake route (#115).
 *
 * Three lines, and this suite is about all three: the gate is asked first, what it returns is what
 * the reader is given, and the roles it resolved decide the head's two controls — **Re-estimate
 * all** drawn for an owner or an admin and nobody else, **Queue N selected ⟳** inert for a viewer.
 * The copy and the presses are covered where they are decided.
 */
...
vi.mock("@/app/issues/data", () => ({ readIssues: (access: unknown) => readIssues(access) }));
...
describe("the intake route", () => {
  ...
  it("hands the reader exactly what the gate resolved", async () => {
    const resolved = access();
    requireWorkspace.mockResolvedValue(resolved);

    await Page();

    expect(readIssues).toHaveBeenCalledExactlyOnceWith(resolved);
  });
