...
describe("the seeded entries", () => {
  ...
  it("has exactly four built destinations today: the dashboard, Issues, Models and Settings", () => {
    ...
    // Issues with #115 (`app/(app)/issues/page.tsx`) and Settings with #141 — whose
    // `app/(app)/settings/page.tsx` redirects to the section's one built tab, Ticket sources,
    // until #491 builds the hub; the other seven wait for #49 or for their own roadmap issue.
    ...
    ).toEqual([DASHBOARD_PATH, ISSUES_PATH, MODELS_PATH, SETTINGS_PATH]);
  });
