...
import { SELECTED_TRIO, backlogListing, fanout, queuedSelection } from "../helpers/issues";
...
const { BACKLOG_ALREADY_ESTIMATING_CODE, ESTIMATION_RATE_LIMITED_CODE, FORBIDDEN_CODE, backlog } =
  await import("@/app/api/backlog");

/**
 * The backlog resource file (#115) — the three `backlog` operations the intake page head calls.
 *
...
describe("the codes the head's actions branch on", () => {
  ...
  it("appear in ouroboros-rest/openapi.yaml, so a rename there is a failure here", () => {
    for (const code of [FORBIDDEN_CODE, BACKLOG_ALREADY_ESTIMATING_CODE, ESTIMATION_RATE_LIMITED_CODE]) {
