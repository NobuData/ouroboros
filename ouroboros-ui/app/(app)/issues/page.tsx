 ...
 * component draws it. The decisions are in [`app/issues/view.ts`](../../issues/view.ts) and
 * [`app/issues/filter.ts`](../../issues/filter.ts), and the read in
 * [`app/issues/data.ts`](../../issues/data.ts), all covered directly.
 *
 ...
 *
 * ### Why the filter is read here rather than in the browser
 *
 * The filter bar ([#116](https://github.com/NobuData/ouroboros/issues/116)) keeps every control in
 * `?repo=&labels=&state=&sort=&q=` (decision K8), and the query string is read on the **server**, so
 * the first paint is already the filtered view and the bar already shows what was asked. A client
 * component reading it with `useSearchParams` would render the default bar first, would need a
 * `Suspense` boundary to be prerendered at all, and would answer *which view?* one frame later than
 * the page could have — the argument `app/(app)/models/(routing)/page.tsx` makes for `?route=`.
 ...
 *
 * @param props.searchParams The URL's query, which carries the filter bar's state.
...
export default async function Page({
  ...
  const readings = await readIssues(access, filter);
