...
BRAND
  ...
  # A target with parentheses in its path — a Next.js route group — which markdown can
  # only link in the angle-bracket form. In the fixture so that a link reader regressing
  # to plain-form-only turns the conforming tree red, not just some future checkout.
  mkdir -p "$fixture/ouroboros-ui/app/(app)"
  : > "$fixture/ouroboros-ui/app/(app)/layout.tsx"
...
The icons are linked from [`layout.tsx`](<app/(app)/layout.tsx>).
...
check_break 'an angle-bracketed link to a file that does not exist is reported' \
  'the link to app/\(app\)/GONE\.tsx resolves' \
  'printf "\n[gone](<app/(app)/GONE.tsx>)\n" >> "$root/ouroboros-ui/README.md"'
